...
compile: <nil>
compile: <nil>
compile: (if (paren (! (~ ($# *) (words <nil> 1)))) (brace (; (>[1=2] (simple (arglist (arglist echo 'usage: unmount mtpt') <nil>))) (simple (arglist exit usage)))))
compile: (f=(` (brace (| (simple (arglist u mount)) (simple (arglist grep ($ 1))))) <nil>)
compile: (if (paren (&& (| (simple (arglist echo ($ f))) (simple (arglist (arglist (arglist 9 grep) -s) 'type fuse')) (! (~ (` (brace (simple uname))) (words <nil> Darwin))))) (simple (arglist (arglist (arglist (arglist exec fusermount) -u) -z) ($ 1))))
compile: (simple (arglist (arglist (arglist exec u) umount) ($ 1)))
