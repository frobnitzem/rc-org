...
compile: (if (paren (! (~ ($ XOFF) (words (words <nil> '') -x*)))) (XOFF=(^ -x ($ XOFF)) <nil>))
compile: (if (paren (! (~ ($ YOFF) (words (words <nil> '') -y*)))) (YOFF=(^ -y ($ YOFF)) <nil>))
compile: (| (simple (arglist (arglist (arglist (arglist (arglist (arglist (arglist (arglist (arglist eval tr2post) ($ XOFF)) ($ YOFF)) ($ COPIES)) ($ MAG)) ($ NPAG)) ($ LAND)) ($ OLIST)) ($ PATCH))) (simple (^ ($ LPLIB) /process/hpost))
compile: (simple exit)
