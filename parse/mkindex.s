...
compile: <nil>
compile: <nil>
compile: (>lookman.index <nil>)
compile: (for i (words <nil> (^ ($ PLAN9) /man/man[0-9]*/[a-z0-9:]*.[0-9]*)) (brace (| (| (| (| (<($ i) (simple (arglist (arglist deroff -w_) <nil>))) (simple (arglist (arglist tr 'A-Z') 'a-z')) (simple (arglist sort -u)) (simple (arglist (arglist (arglist comm -23) -) junkwords)) (>>lookman.index (simple (arglist (arglist sed (^ (^ 's@$@	' ($ i)) '@')) <nil>)))))
compile: (simple (arglist (arglist (arglist sort -o) lookman.index) lookman.index))
