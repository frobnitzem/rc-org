...
compile: <nil>
compile: <nil>
compile: (fn (words <nil> local) (brace (; (GSDEVICE=(` (brace (| (simple (arglist echo ($ LPCLASS))) (simple (arglist (arglist 9 sed) 's/(.*\+)?gs!([^+]*)(\+.*)?/\2/')))) <nil>) (; (simple (arglist (arglist echo gs) ($ GSDEVICE))) (>[2]($ PRINTLOG) (brace (| (<(^ (^ ($ LPDEST) /) ($sub FILE (words <nil> 1))) (simple (arglist (arglist (arglist (arglist (arglist psdownload -f) (^ (^ -H ($ PLAN9)) /postscript/font)) -mfontmap) -plw+) <nil>))) (simple (arglist (arglist (arglist (arglist (arglist (arglist (arglist gs (^ (^ -sDEVICE '=') ($ GSDEVICE))) (^ (^ -s 'OUTPUTFILE=') ($ OUTDEV))) -dSAFER) -dNOPAUSE) -dQUIET) -dBATCH) -))))))))
