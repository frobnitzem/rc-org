...
%%EndFeature') <nil>))))
compile: (PATCH=(^ ($ PATCH) '
%%EndPatch from lp') <nil>)
compile: (if (paren (! (~ ($ PATCH) (words (words <nil> '') -P*)))) (PATCH=(^ -P ($ PATCH)) <nil>))
compile: <nil>
compile: (if (paren (~ ($ MAG) (words <nil> ''))) (MAG=1 <nil>))
compile: (if (paren (~ ($ MAG) (words <nil> [.0-9]*))) (MAG=(^ (^ (^ -m ($ MAG)) ,) (` (brace (| (simple (arglist (arglist (arglist echo ($ MAG)) '*') 2)) (simple hoc)))) <nil>))
compile: <nil>
compile: (if (paren (! (~ ($ LAND) (words <nil> '')))) (LAND=-L <nil>))
compile: (if not (LAND=(paren <nil>) <nil>))
compile: <nil>
compile: (| (| (simple g3p9bit) (simple (arglist (arglist (arglist p9bitpost ($ MAG)) ($ LAND)) ($ PATCH))) (simple (^ ($ LPLIB) /process/hpost))
compile: (simple exit)
