...
compile: <nil>
compile: (if (paren (! (~ ($ MAG) (words <nil> '')))) (MAG=(^ -m ($ MAG)) <nil>))
compile: (if (paren (~ ($ MAG) (words <nil> ''))) (MAG=(paren <nil>) <nil>))
compile: <nil>
compile: <nil>
compile: <nil>
compile: (| (simple (arglist (arglist (arglist p9bitpost ($ MAG)) ($ LAND)) ($ PATCH))) (simple (^ ($ LPLIB) /process/hpost))
compile: (simple exit)
