...
compile: <nil>
compile: (simple (arglist (arglist builtin cd) ($ 1)))
compile: (for i (words <nil> [a-z0-9:]*.[0-9]*) (| (brace (; (b=(` (brace (| (simple (arglist echo ($ i))) (simple (arglist sed 's/\..*//')))) <nil>) (; (simple (arglist (arglist (arglist (arglist 9 sed) -n) (^ (^ (^ (^ '
	/SH *NAM/,/SH/{
		/SH/d
		s/, *$//
		ty
...
/g
	...
	p
	q
')) ($ i))) (simple (arglist (arglist echo ($ b)) ($ i)))))) (simple (arglist (arglist 9 sort) -u)))
