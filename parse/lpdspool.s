...
compile: <nil>
compile: (for j (words <nil> ($ *)) (brace (; (if (paren (|| (~ ($ j) (words <nil> '')) (simple (arglist (arglist test -f) ($ j))))) (brace (| (simple (arglist (arglist (arglist (arglist lpinput ($ j)) lpsub) process) ($ LPPROC))) (>[2](^ (^ (^ ($ LPLOGDIR) /) ($ LPDEST)) .st) (simple (arglist (arglist (arglist (arglist (arglist (arglist lpdsend (^ -d ($ OUTDEV))) -tf) (^ -H ($ LPMACHID))) (^ -P ($ LPUSERID))) ($ DEST_HOST)) <nil>))))) (if not (brace (>[1=2] (simple (arglist (arglist (arglist (arglist (arglist echo ($ j)) cannot) be) opened) <nil>))))))))
