...
compile: <nil>
compile: <nil>
compile: (if (paren (~ ($# *) (words <nil> 0))) (&& (simple (arglist (arglist (arglist echo usage:) unspambox) mailfs/mbox)) (simple (arglist exit "usage"))))
compile: (box=($ 1) <nil>)
...
compile: (for m (words <nil> ($ msgs)) (brace (; (simple (arglist (arglist echo -n) (^ ' ' ($ m)))) (simple (arglist unspam (^ (^ ($ box) /) ($ m)))))))
