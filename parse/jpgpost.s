...
compile: (if (paren (~ ($ MAG) (words <nil> [.0-9]*))) (MAG=(^ -m ($ MAG)) <nil>))
compile: <nil>
compile: (if (paren (! (~ ($ LAND) (words <nil> '')))) (LAND=-L <nil>))
compile: (if not (LAND=(paren <nil>) <nil>))
compile: <nil>
compile: (| (| (simple (arglist (arglist 9 jpg) -t9)) (simple (arglist (arglist (arglist (arglist (arglist (arglist p9bitpost ($ MAG)) ($ LAND)) ($ PATCH)) -p) 32) 26.17)) (simple (^ ($ LPLIB) /process/hpost))
compile: (simple exit)
