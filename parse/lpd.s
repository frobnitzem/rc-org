...
compile: <nil>
compile: (simple (arglist (arglist echo transmission) log:))
compile: (if (paren (simple (arglist (arglist test -r) (^ (^ (^ ($ LPLIB) /log/) ($ LPDEST)) .st)))) (brace (; (simple (arglist (arglist tail -3) (^ (^ (^ ($ LPLIB) /log/) ($ LPDEST)) .st))) (simple echo))))
compile: (simple (arglist (arglist echo printer) queue:))
compile: (simple (arglist (arglist (arglist lpdsend -q) (^ -d ($ OUTDEV))) ($ DEST_HOST)))
