...
compile: <nil>
compile: (if (paren (! (~ ($ DEBUG) (words <nil> '')))) (brace (simple (arglist (arglist flag x) +))))
compile: (if (paren (simple (arglist (arglist test -e) /net/tcp/clone))) (brace (; (dialstring=(` (brace (simple (arglist (arglist (arglist ndb/query sys) ($ 1)) dom)))) <nil>) (; (network=tcp <nil>) (; (if (paren (|| (~ ($# dialstring) (words <nil> 0)) (! (~ ($ dialstring) (words <nil> ''))))) (brace (dialstring=($ 1) <nil>))) (; (if (paren (simple (arglist lpsend (^ (^ (^ ($ network) !) ($ dialstring)) !printer)))) (simple (arglist exit ''))) (rv='tcp failed' <nil>)))))))
compile: (if not (rv='no tcp' <nil>))
compile: <nil>
compile: <nil>
compile: (if (paren (! (~ ($ dialstring) (words <nil> '')))) (simple (arglist exit 'lpsend: no dialstring')))
compile: (if not (simple (arglist exit (^ 'lpsend: ' ($ rv)))))
